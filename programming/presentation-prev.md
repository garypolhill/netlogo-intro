...
+ "Things" are the objects and agents that make up your model
+ Different categories of thing are called **breeds** in NetLogo
...
# Our running example this session

+ **Infectious disease transmission in a workplace**
+ We'll build this model up piece by piece as we go through today's ideas
+ Right now: just declare some breeds you think you might need

---

# Exercise

+ Declare some breeds for the workplace disease model, using `breed [ ]`
+ Check the syntax
+ Goal: just to practice writing code and checking it's correct -- there's no single right answer yet
...
+ Canteen
+ Meeting Room
+ Corridor, Light Switch, Toilet

</div>
</div>

<p class="fragment">Discussion: are ontologies <em>discovered</em> or <em>constructed</em>? How context-sensitive are they -- would a hospital's list look the same?</p>
...
+ The **world** on the Interface tab is divided into square cells called **patches**
  ...
  + Gives spatial locations for agents -- useful for visualization even in models without real space
...
# Let's create some of these agents

```netlogo
create-breed-people number-of-people [
  move-to one-of patches
]
```

+ `create-breed [ ]` creates new agents of a breed
+ `move-to` places an agent somewhere
...
+ Two purposes:
  + **State variables** -- how do you tell one agent apart from another? What makes this one different?
  + **Useful computational variables** -- save time by storing a value with the agent instead of recomputing it (a general time/memory trade-off in computing)
...
+ `turtles-own [ ]` -- attributes of *all* agents
+ `breeds-own [ ]` (plural breed name) -- attributes of one particular breed
+ `patches-own [ ]` -- attributes of patches

<p class="fragment">Indent your code consistently -- it makes it far more readable. Develop a style and stick to it.</p>
...
+ Random heading, random colour, `"default"` shape
+ `xcor` and `ycor` start at 0 -- remember we had to use `move-to` to get agents somewhere else
...
+ Add attributes to your breeds -- make them up for now
+ Give your `persons` (and any other breeds) attributes you think might matter for a disease-transmission model
+ Remember: `turtles-own [ ]`, `breeds-own [ ]` (plural!), `patches-own [ ]`, kebab-case names
+ You can comment your variable names to explain them
...
|---|---|
| `n-` | "number of" |
| `p-` | "probability of" / "proportion of" |
| `pct-` | "percentage" |
...
# Now let's initialize some attributes
...
+ All `-own` attributes are 0 unless you set them -- **recommendation: always initialize**
+ **Literals** are fixed values: Booleans (`true`/`false`), numbers (`0`, `1`), strings (`"person"`), colours (`pink`, `blue`, `green` -- see Tools >> Color Swatches)
+ Use `set` to assign a variable a value
...
```netlogo
ask persons with [ not infected? ] [
  set color green
]

show count persons with [ infected? ]
```

+ `ask` tells a set of agents to do something
+ `with [ ]` selects a subset -- put a Boolean expression inside, using `=`, `>`, etc.
+ `count` tells you how many members a set has
+ `breeds-here` tells you what's on the current patch

---

# "Global" attributes

+ Sometimes it helps to have a variable accessible from *anywhere* -- not just turtles, patches or links
+ Useful for: caching expensive-to-calculate values, properties of the whole "world", filenames, time series
+ Some programmers frown on them -- they make code more interdependent
+ Declare at the top of your code:

```netlogo
globals [ n-infections-total ]
```

---

# Relations -- the theory
...
+ Can be thought of as a matrix of Booleans -- true where the relation holds, false where it doesn't (symmetric relations only need a triangular matrix)
...
+ NetLogo calls relations **links** -- generally used for social networks
+ All links are **irreflexive** -- an agent cannot link to itself
...
+ **Not (necessarily) symmetric**: `directed-link-breed [ plural singular ]`
  + Not the same as *anti*symmetric -- if A manages B on one project and B manages A on another, they're not the same person!

---

# Exercise

+ Declare some link breeds for your workplace infection model
+ What relations might matter in a workplace?
+ Are they directed, or undirected?

<p class="fragment">One answer: <code>undirected-link-breed [ colleagues colleague ]</code>, <code>directed-link-breed [ manages manage ]</code></p>
...
+ From a software perspective, what actually *makes* something an "agent"? Autonomy, goals, reasoning, acting on behalf of a human, special interaction protocols…
...
+ `link-breeds-own [ ]` -- gives relations their own attributes
+ Computer scientists call this "reifying" a relation -- nothing to worry about in NetLogo, just a useful trick
...
# Let's build a workplace social network

**Management hierarchy** -- for each person, think from their own perspective:

+ Who could manage *me*? If I'm not managed yet, and someone could manage me, pick one
+ Who could *I* manage? If I don't manage anyone, and someone needs managing, pick one

+ There are much easier ways to build networks (the `nw` extension helps a lot) -- but we're doing it by hand once, deliberately, as it exercises everything covered today
...
+ `ask` a group of agents to do something <!-- .element: class="fragment" -->
+ `let` creates a temporary variable -- here, `management` <!-- .element: class="fragment" -->
+ `other persons` -- everyone except the agent running the code <!-- .element: class="fragment" -->
+ `with [ ]` selects a subset; `out-manage-neighbors` follows a directed link outward; `any?` is true if the set isn't empty <!-- .element: class="fragment" -->
+ `if [ ]` runs code only when a Boolean expression is true <!-- .element: class="fragment" -->
+ `create-manage-from` makes a directed link **to** us **from** someone else <!-- .element: class="fragment" -->

--

## Finishing the hierarchy, and showing it off
...
# Three things every language provides

Let's look again at the network-building code -- there are three ingredients every programming language needs:
...
+ Doing one thing, finishing it, moving on to the next -- one or more instructions, in order
+ In our code: define a temporary variable, then check-and-create a link, then define another temporary variable, then check-and-create another link

---

# Selection

+ Allowing a sequence to run only under restricted conditions
+ In our code: `with [ ]` restricts a set under a condition; the `if [ ]` around `create-manage-from`/`create-manage-to` only runs when its Boolean expression is true
...
(ifelse
  ...
  [
    ; deliberately empty -- keep going with the rest of go
  ]
)
```

+ `if [ ]` -- do something if a condition is true
+ `ifelse [ ] [ ]` -- do one thing if true, another if false
+ `(ifelse [ ] [ ] … [ ])` -- multiple conditions; always test the more specific ones first

---

# Iteration

+ Repeatedly executing a sequence
+ In our code: `ask persons [ ]` asks *each* person to do something -- that's iteration
+ `with [ ]` also has to look at every candidate agent to select from -- selection **and** iteration in one command
+ When one iteration sits inside another (as it does here), that's **nested iteration**
...
+ Nested iteration means run-time grows with the **square** of the number of agents

| n-persons | nested steps (n²) | at 1 microsecond each |
|---|---|---|
| 100 | 10,000 | 0.01 seconds |
| 10,000 | 100,000,000 | 100 seconds |
| 1,000,000 | 10¹² | ~11.5 days |

+ It's also the first place to look when you need to speed a model up
+ Computing time isn't the only cost -- storage space matters too
...
while [ count persons with [ infected? ] > 0 ] [
  go
]
```

+ `repeat [ ]` -- when you know exactly how many times
+ `while [ ] [ ]` -- when you don't; repeats the second brackets while the first stays true
  + Make sure the condition really can become false, or you'll loop forever
  + **Tools >> Halt** is your friend if it doesn't

---

# Coming up

+ We've named things, found our way round the Code tab, and given our workplace model breeds, attributes and relations
+ This afternoon (Session 2): reporters, strings, lists, Booleans, maths, procedures, and finally getting agents moving -- turning today's static network into a running simulation
